...
void ParticleSystem::Emit(Vector2f position) {
	for (int i = 0; i < (int)emissionPerFrame; i++) {
		double properAngle;
		if (angle < 0) {  // Emits in a circle
		...
		double properRadius;
		if (radiusVariation) {
			properRadius = radius + rand() % (int)radius;
		}
		...
		if (transparency) particle.SetTransparency(true);
		else particle.SetTransparency(false);
...
void ParticleSystem::SetAngle(double newAngle) {
	angle = newAngle;
}
...
void ParticleSystem::SetEmission(double emission) {
	emissionPerFrame = emission;
}

void ParticleSystem::SetRadius(double newRadius) {
	radius = newRadius;
}

void ParticleSystem::SetTransparency(bool value) {
	transparency = value;
}
